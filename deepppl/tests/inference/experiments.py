...
def crunch(dirname):
    ...
    for x in name_map:
        results = {'timers': {}}
        divergences = {'pyro': {}, 'numpyro': {}}
        pyro = defaultdict(list)
        numpyro = defaultdict(list)
        timers = defaultdict(list)
        files = [os.path.join(dirname, f) for f in os.listdir(dirname) if f.startswith(f'test_{x}')]
...
def to_tex(summary):
    ...
    print(r'&\multicolumn{2}{c}{\textsc{Stan}}& \multicolumn{3}{c}{\textsc{DeepStan/Pyro}} & \multicolumn{3}{c}{\textsc{DeepStan/NumPyro}}\\')
    print(r'\cmidrule(lr){2-3}')
    print(r'\cmidrule(lr){4-6}')
    print(r'\cmidrule(lr){7-9}')
    print(r'&\multicolumn{2}{c}{time(s)} & time(s) & \multicolumn{2}{l}{KS p-value} & time(s) & \multicolumn{2}{l}{KS p-value}\\')
    print(r"\toprule")
    for t in name_map:
        if t == 'aspirin':
            print(r'\midrule')
        ...
        t_numpyro = f"{d['timers']['NumPyro_Runtime']:,.0f}"
        ...
        if 'Pyro_Runtime' in d['timers']:
            t_pyro = f"{d['timers']['Pyro_Runtime']:,.0f}"
            ...
            t_pyro = r"\_"
            ks_pyro = r"\_ & \_"
        print(f"{name} & {t_stan} & {t_pyro} & {ks_pyro} & {t_numpyro} & {ks_numpyro}\\\\")
    print(r"\bottomrule")
    print(r"\end{tabular}")
        

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compare the output of NUTS for DeepStan (with Pyro and Numpyro) and Stan on the following experiments: coin, double normal, reparameterization, linear regression, aspirin, roaches, 8 schools, seeds')
