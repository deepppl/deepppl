...
class PyroModel(object):
    def __init__(self, model_code=None, model_file=None, pyro_file=None, **kwargs):
        self._scope = kwargs
        self._py = self.compile(
            model_code=model_code, model_file=model_file, pyro_file=pyro_file
        )
        self._load_py()
    ...
    def mcmc(
        self, num_samples=10000, warmup_steps=1000, num_chains=1, thin=1, kernel=None
    ):
        if kernel is None:
        ...
        return MCMCProxy(
            mcmc, False, self._generated_quantities, self._transformed_data, thin
        )
...
class NumPyroModel(PyroModel):
    def _stan_scoped(self, name):
        ...
        scoped["softplus"] = lambda x: jnp.logaddexp(x, 0.0)
        scoped["fabs"] = jnp.abs
        scoped.update(self._scope)
        f = self._scope[name]
        scoped[name] = FunctionType(f.__code__, scoped, name)
    ...
    def mcmc(
        ...
        return MCMCProxy(
            mcmc, True, self._generated_quantities, self._transformed_data, thin
        )
